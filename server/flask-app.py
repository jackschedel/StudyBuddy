# flask --app ./server/flask-app.py run
...
def upload_from_url():
    try:
        ...
        logging.info(
            f"File {filename} fetched from URL and uploaded to Pinecone successfully."
        )
...
def upload_file():
    try:
        ...
        if file and allowed_file(file.filename):
            ...
            os.remove(filepath)
            logging.info(
                f"File {file.filename} processed and uploaded to Pinecone successfully."
            )
            return jsonify({"message": "Uploaded successfully!"}), 200

        else:
            logging.warning(
                f"Attempted upload of unsupported file type: {file.filename}"
            )
...
def tokenize_text(docs):
    ...
    for doc in tqdm(docs):
        ...
        for i, chunk in enumerate(chunks):
            documents.append({"id": f"{uid}-{i}", "text": chunk})
...
def generate_questions():
    try:
        ...
        logging.error(f"Error querying agent: {e}")
        return (
            jsonify({"error": "An unexpected error occurred querying the agent"}),
            500,
        )
